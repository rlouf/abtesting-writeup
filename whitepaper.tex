...
It is known (and easy to calculate) that the distribution of $t_x$,
the sum of $x$ exponentially distributed intervent times, is:
\begin{align*}
...
being active at time $\tau \in \left] t_x, T \right]$ with no event
    between $t_x$ and $\tau$. That is to say:
\begin{align*}
...
doesn't allow for explicit differentiation.
