...
The number of users still present at time $t$ is the random variable $n_t$ defined as:
...
follows a shifted geometric distribution:
...
Computing for all times t in one go is complicated so we instead progress recursively:

\begin{equation}
  P(n_0 = N_0 | \theta, \alpha, \beta) = \left(1-\theta_i\right)
\end{equation}
...
\begin{equation}
  P(\alpha, \beta | N_t) \propto \left[ \prod_{i=1}^N \left(\int_0^1  S_i(t)
  P(\theta_i | \alpha, \beta) \mathrm{d}\theta_i \right)
  \right] P(\alpha, \beta)
\end{equation}

The integrals over $\theta_i$ are:

\begin{align*}
  I_i &= \int_0^1 \left(1-\theta_i\right)^t \frac{\theta_i^{\alpha-1} \left(1-\theta_i\right)^{\beta-1}}{B(\alpha,
...
\begin{equation}
  P\left(\alpha, \beta | N_t\right) = \left[S_t(\alpha, \beta)\right]^N P(\alpha, \beta)
\end{equation}
