...
The Pareto/NBB model is based on the following assumptions:
\begin{itemize}
...
We are going to use MCMC methods to estimate the parameters $r,\alpha,
s, \beta$.
...
Observing $x,t_x, T$ lets us with two possibilities: either the user
is still active at time $T$ with no events between $t_x$ and $T$ (with
probability $e^{-\lambda (T-t_x)} e^{-\mu T}$), or he has stopped
being active at time $\tau \in \left] t_x, T \right]$ with no event
between $t_x$ and $\tau$. That is to say (for $x\ne 0$):
\begin{align*}
...
marginalised) is derived. However this expression is complex and
doesn't allow for explicit differentiation, preventing us from using
NUTS sampler. However the independence assumed between $\lambda$ and
$\mu$ makes variational inference suitable.
...
\paragraph{Probability of being alive at $T$}
\label{subsub:proabilityofbeingalive}

\paragraph{Expected activites in interval $[T, T+t]$}
\label{subsub:expectedactivtiesin interval}
